...
// 전역 변수 및 구조체 선언
ProcessList readyQueue;         // 준비 큐
IntervalList workingQueue;      // 작업 큐
Gant gant[62];                  // 간트 차트 배열
...
void* process_thread(void* arg) {
    while (1) {
        ...
        // 도착한 프로세스 중 우선순위가 가장 높은 프로세스 찾기
        while (p != NULL) {
        ...
        // 활성 프로세스가 없는 경우에만 새로운 프로세스를 선택
        if (active_pkt == NULL || active_pkt->remaining_time == 0) {
