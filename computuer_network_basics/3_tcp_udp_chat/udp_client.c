...
7. pthread_create 는 포인터를 다 받기 위해서 void * 타입을 사용함.(generic pointer)
*/
...
void *sendToServer(void *arg) {
    struct sockaddr_in *serveraddr = (struct sockaddr_in *)arg; // 타입 캐스팅 필요함.
    char buf[BUFSIZE+1];
    while (1) {
        fgets(buf, BUFSIZE, stdin);
        if (sendto(sock, buf, strlen(buf), 0, (struct sockaddr *)serveraddr, sizeof(struct sockaddr_in)) < 0) {
            perror("send failed");
            break;
        }
        printf("Client: %s", buf);
...
void *receiveFromServer(void *arg) {
    ...
    char buf[BUFSIZE+1];
        while (1) {
        ssize_t received = recvfrom(sock, buf, BUFSIZE, 0, (struct sockaddr *)serveraddr, &addr_len); 
        if (received > 0) {
            buf[received] = '\0';
            printf("Server: %s", buf);
        } else {
...
int main(int argc, char *argv[]) {
    ...
    inet_pton(AF_INET, SERVERIP, &serveraddr.sin_addr); // 서버 IP 연결

    printf("host join this chatting room.\n");

    // 쓰레드 2개 생성(pid)
    pthread_t thread_send, thread_recv;
    pthread_create(&thread_send, NULL, sendToServer, (void *)&serveraddr);
    pthread_create(&thread_recv, NULL, receiveFromServer, (void *)&serveraddr);
